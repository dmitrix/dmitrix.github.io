...
var TOP_SPEED = 500;     // The fastest the player can move
var SCALE_RATE = 0.002;  // How fast the player looses mass
var SCALE_MIN = 0.01;    // Burn out Point
var SCALE_MAX = (window.innerWidth-50) / (350);       // Black Hole Point
...
function preload(){
  ...
  /* Load Sounds */
  game.load.audio('select', 'res/select.ogg');
  game.load.audio('consumed', 'res/consumed.ogg');
  game.load.audio('deathshrink', 'res/deathshrink.ogg');
  game.load.audio('selecthi', 'res/selecthi.ogg');
  game.load.audio('blackholeSound', 'res/blackhole.ogg');
...
function create(){
  ...
  consumed = game.add.audio('consumed');
  deathshrink = game.add.audio('deathshrink');
  selecthi = game.add.audio('selecthi');
  blackholeSound = game.add.audio('blackholeSound');
...
function gm(blackhole)
{
  ...
  else
  {
    if (initGM)
    {
      ...
      game.add.tween(player.scale).to( { x: 0.5, y: 0.5 }, 2000, Phaser.Easing.Linear.None, true, 0, 0, false);
      game.add.tween(blackHole).to( { alpha: 1 },  2000, Phaser.Easing.Linear.None, true, 0, 0, false);
      game.add.tween(blackHole.scale).to( { x: 0.5, y: 0.5 }, 2000, Phaser.Easing.Linear.None, true, 0, 0, false);
